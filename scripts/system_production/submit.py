...
def gen_sub(jobstep):
 ...
 script_name = "gpuprod-%d.sh" % jobstep
...
#SBATCH -J rsmc-p%d  # Job name
#SBATCH -o log/rotatedSmallChargedMembrane_2-ions-wats-prod%d.out          # Name of stdout output file

export jobstep='000%d'
export prev_jobstep='000%d'
...
# Run NAMD
namd3 +p15 +pmepes 5 +setcpuaffinity +devices 0,1,2,3 equil_rcmnpt.conf > log/${basename}-${jobstep}.log   # with 8 PEs per non-PME


""" % (jobstep, jobstep, jobstep, jobstep - 1))
 output.close()
...
for cycle in range(start, end + 1):
 ...
 if cycle == start:
     ...
     jobid = submit(script, f"--dependency=afterany:{jobid}")
