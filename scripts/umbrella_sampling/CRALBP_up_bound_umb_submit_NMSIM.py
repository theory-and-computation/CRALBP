...
#SBATCH -o log/cont_upperpore-umb-%d.out          # Name of stdout output file

export window='%d'
export base_jobstep='00007'
...
export basename='rotatedTNeutralMembrane_act-ions-wats'
export nsteps='5500000'
export temp='309.2162'

export basedir='output'
export rstname=$basedir'/'${basename}-${base_jobstep}'/'${basename}-${base_jobstep}'.restart'

export rundir=$basedir'/contup_'${basename}-${window}
export output=$rundir'/contup_'${basename}-${window}
...
# Run NAMD
namd3 +p29 +pmepes 5 +setcpuaffinity +devices 0,1,2,3 cont_up-%d.conf > log/'up_'${basename}-${window}.log   # with 8 PEs per non-PME
